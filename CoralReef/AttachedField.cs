...
namespace CoralReef {
    /// <summary>
    /// A collection that attaches values to objects using <see cref="WeakReference"/>. From Slime_Cubed
    /// </summary>
    /// <remarks>
    ...
    public class AttachedField<TKey, TValue> {
        ...
        public TValue Get(TKey obj) {
            ...
            if (_dict.TryGetValue(obj, out TValue value)) 
                return value;
        ...
        /// <summary>
        /// Checks for a stored value for a given object.
        /// </summary>
        /// <param name="obj">The object to get from.</param>
        /// <returns>True if a value exists for <paramref name="obj"/>, false otherwise.</returns>
        public bool TryGetNoVar(TKey obj) 
            => TryGet(obj, out _);
        ...
        public void CullDead() {
            ...
            foreach (KeyValuePair<object, TValue> pair in _dict) {
                if (pair.Key is not WeakReference wr || (!wr.IsAlive)) 
                    _toRemove.Add(pair);
            }
